...
object stackMachine {
  ...
  object Times {
    def apply() = new Times()
  };import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(2345); 
  // ---------- Sample Run ----------
  StackMachine.program = List(Push(3), Push(4), Push(5), Sum(), Times(), Top());$skip(21); 
  StackMachine.run();$skip(84); 

  StackMachine.program = List(Push(10), Push(10), Times(), Push(20), Sum(), Top());$skip(21); 
  StackMachine.run()}
